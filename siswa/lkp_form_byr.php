        ...
        if($status=="Siswa")
            {
                ...
                <head>
                   ...
                   <meta charset="utf-8">
                    <title>Formulir Kelengkapan Pembayaran</title>
                    <meta name="keywords" content="Formulir Kelengkapan Pembayaran" />
                    <meta name="description" content="Formulir Kelengkapan Pembayaran">
                    <meta name="author" content="Formulir Kelengkapan Pembayaran">
                                ...
                                <div class="topbar-left">
                                    <ol class="breadcrumb">
                                        <li class="crumb-active">
                                            <a href="lkp_form">Formulir Kelengkapan Pembayaran</a>
                                        </li>
                                    </ol>
                                </div>
                            ...
                            if(isset($_POST['tb_update']))
                                {
                                    ...
                                    $bukti_transferty=$_FILES['bukti_transfer']['type'];
                                    $bukti_transfernm=$_FILES['bukti_transfer']['name'];

                                    
                                    $tp_dt_ft=mysqli_fetch_row(mysqli_query($conn,"Select bukti_transfer from tb_pembayaran where id_siswa='$fi_id'"));
                                    $ck=0;
                                    if($bukti_transferty=="image/jpeg" || $bukti_transferty=="image/jpg" || $bukti_transferty=="image/png")
                                    {
                                        $ck=1;
                                        $fn_pasfoto=$bukti_transfernm;
                                    }
                                    else
                                    {
                                        $ck=2;
                                    ...
                                    $cek_data=mysqli_num_rows(mysqli_query($conn,"Select * from tb_pembayaran where id_siswa='$fi_id'"));
                                    if($cek_data>0)
                                    {
                                         mysqli_query($conn,"Update tb_pembayaran set nominal='$nominal', nama_pengirim='$nama_pengirim', no_rek_pengirim='$no_rek_pengirim', bank_pengirim='$bank_pengirim', bukti_transfer='$fn_pasfoto', tgl_entry='$date_update', status='Baru', status_pb='$metode' where id_siswa='$fi_id'");
                                    }
                                    else
                                    {

                                        mysqli_query($conn,"Insert into tb_pembayaran set id_siswa='$fi_id',nominal='$nominal', nama_pengirim='$nama_pengirim', no_rek_pengirim='$no_rek_pengirim', bank_pengirim='$bank_pengirim', bukti_transfer='$bukti_transfernm', tgl_entry='$date_update', status='Baru', status_pb='$metode'");
                                    }

                                    if($ck==1)
                                    {
                                         move_uploaded_file($bukti_transfer['tmp_name'], "foto_trf/".$fn_pasfoto);
                                    }
                                    ...
                                    <strong>Sukses!</strong> Selamat Anda Mengirimkan
                                    <a href="#" class="alert-link"> Formulir Pembayaran</a>.</div>
                                    <meta http-equiv="refresh" content="5;url=lkp_form_byr">
                                    <?php
                                    include("form_lkp_byr.php");
